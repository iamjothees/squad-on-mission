...
<div class="{{ $wrapperClass }}" wire:ignore x-data="{
 ...
 <select x-ref="select" {{ $attributes->merge(['class' => 'w-full bg-bg border border-border rounded-md px-3 py-2 text-fg focus:outline-none focus:ring-2 focus:ring-accent']) }} {{ $multiple ? 'multiple' : '' }}>
 {{ $slot }}
 </select>
</div>
