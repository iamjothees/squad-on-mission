<x-layouts.app title="Ideas">
 ...
 <div>
 <h1 class="font-bold text-fg text-lg">Ideas</h1>
 <p class="text-xs text-fg-muted mt-1">Dump and organize your crazy ideas, brainstorms, and someday-projects.</p>
 </div>
 <a href="{{ route('ideas.create') }}" wire:navigate class="bg-accent text-accent-fg text-accent-fg px-4 py-2 rounded-md text-sm font-semibold hover:opacity-90 hover:opacity-90 transition-colors shadow-sm">
 + New Idea
 </a>
 </div>
 ...
 <div class="bg-surface border border-border rounded-lg shadow-sm p-4 flex flex-col hover:border-border dark:hover:border-gray-700 transition-colors group">
 <div class="flex justify-between items-start mb-2">
 <h4 class="font-medium text-fg text-sm leading-tight group-hover:text-accent transition-colors">{{ $idea->title }}</h4>
 ...
 @else bg-surface0/10 text-fg-muted
 @endif
">
 {{ $idea->status }}
 </span>
 ...
 @if($idea->description)
 <p class="text-xs text-fg-muted line-clamp-3 mb-4 flex-grow">{{ $idea->description }}</p>
 @else
 <div class="flex-grow"></div>
 @endif
 
 <div class="flex justify-end items-center mt-4 pt-3 border-t border-border">
 ...
 @empty
 <div class="col-span-full py-12 text-center border border-dashed border-border rounded-lg text-fg-muted text-sm">
